    ...
    view! {
        <section class="workbench">
            ...
            <div class="mini-workbench">
                ...
                <button type="button" class="button primary" on:click=move |_| spawn_local(async move { if let (Ok(pa),Ok(pb))=(curve::generate_x25519().await,curve::generate_x25519().await){ set_a.set(Some(pa)); set_b.set(Some(pb)); set_ephemeral.set(None); set_ephemeral_hex.set(String::new()); set_ciphertext.set(String::new()); set_result.set("Generated fresh recipients A and B.".to_owned()); set_encrypted_seen.set(false); set_a_ok.set(false); set_b_failed.set(false); set_recipient_answer.set(None); } })>"Generate recipients"</button>
            ...
            <div class="workbench-quiz">
                ...
                <div class="hero-actions">
                    <button type="button" disabled=move||!encrypted_seen.get() on:click=move |_| { if let (Some(pair),Some(eph))=(a.get(),ephemeral.get()){ let c=ciphertext.get(); spawn_local(async move { match curve::open_from(&pair.private,&eph,&c).await { Ok(value)=>{set_result.set(format!("A recovered: {value}"));set_a_ok.set(true);}, Err(_)=>set_result.set("A could not decrypt.".to_owned()), } }); } }>"Decrypt with A"</button>
                    <button type="button" disabled=move||!encrypted_seen.get() on:click=move |_| { if let (Some(pair),Some(eph))=(b.get(),ephemeral.get()){ let c=ciphertext.get(); spawn_local(async move { if curve::open_from(&pair.private,&eph,&c).await.is_err(){set_result.set("B failed to decrypt, as expected.".to_owned());set_b_failed.set(true);}else{set_result.set("Unexpectedly decrypted with B.".to_owned());} }); } }>"Decrypt with B"</button>
