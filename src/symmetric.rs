...
pub fn SymmetricEncryptionLesson() -> impl IntoView {
    ...
    view! {
        <LessonIntro number="02" title="Shared-key encryption" eyebrow="Keep the backup private" summary="Encryption turns readable data into ciphertext that only someone with the shared secret key can recover." />
        ...
        <section class="lesson-explanation content-section"><h2>"What did this prove?"</h2><p class="section-copy">"With AES-GCM, someone who doesn't know the shared key should not be able to read the encrypted backup. AES-GCM also detects changes to the encrypted data. But now we have a new problem: if two people want to communicate, how do they safely establish or exchange that secret key?"</p><div class="principles"><article><span>"YES"</span><h3>"Confidentiality"</h3><p>"The ciphertext hides the plaintext from someone who doesn't have the key."</p></article><article><span>"YES"</span><h3>"Tamper detection"</h3><p>"AES-GCM authenticates the encrypted data, so altered ciphertext should fail to decrypt."</p></article><article><span>"NEXT"</span><h3>"Key distribution"</h3><p>"Two people still need some safe way to establish or exchange the shared secret key."</p></article></div></section>
