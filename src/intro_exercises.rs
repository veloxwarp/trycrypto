...
pub fn IntroExercises()->impl IntoView{
 let(a,set_a)=signal(String::new());let(b,set_b)=signal(String::new());let(ra,set_ra)=signal(false);let(rb,set_rb)=signal(false);let done=Memo::new(move |_|ra.get()&&rb.get());
