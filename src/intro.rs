...
pub fn IntroLesson() -> impl IntoView {
    view! {
        ...
        <section class="content-section primer-section">
            <h2>"Why does one extra space change the result?"</h2>
            <p class="section-copy">"To a person, two messages can look identical even if one has an extra space at the end. To a cryptographic algorithm, they are different inputs. Cryptography works with the exact data it receives—including spaces and other characters you may not be able to see."</p>
            ...
            <p class="section-copy">"Long sequences of byte values are awkward to read and copy as ordinary numbers. Hexadecimal gives us a compact display format using 0–9 and A–F. A means 10, and F means 15. Two hex digits can represent every possible byte, from 00 through FF."</p>
            <p class="section-copy">"Hex changes how the bytes are displayed, not the bytes themselves. You'll see it throughout this site whenever you need to copy a key, hash, ciphertext, or signature exactly."</p>
        </section>
